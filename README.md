...
- 현재 기준 약 `42%`
- 이유: 화면 골격, Supabase API, Realtime 구독, 사진 업로드, SQL/RLS 준비 파일에 더해 패키지 설치, 타입검사, Expo Doctor 통과, 작업 목록 현장 필터, 농가-필지-작업 동시 등록 흐름, 환경변수 미설정 안전 진입이 완료됐고 실제 Expo 실행/실기기 테스트가 남아 있습니다.
