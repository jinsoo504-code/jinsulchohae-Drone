...
- 현재 기준 약 `57%`
- 이유: 화면 골격, Supabase API, Realtime 구독, 사진 업로드, SQL/RLS 준비 파일에 더해 패키지 설치, 타입검사, Expo Doctor 통과, 웹 번들 확인, 작업 목록 현장 필터, 농가 검색/선택, 방제팀 생성/배정, 농가-필지-작업 동시 등록, 현재 위치 기반 좌표 입력, 임시 polygon 생성, 환경변수 미설정 안전 진입이 완료됐고 실제 Expo 실행/실기기 테스트가 남아 있습니다.
